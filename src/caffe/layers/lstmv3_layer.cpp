...
#include "caffe/layers/lstmv3_layer.hpp"
...
namespace caffe {
    ...
    template <typename Dtype>
    void LSTMV3Layer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
                                      const vector<Blob<Dtype>*>& top) {
        LSTMV3Parameter lstmv3_param = this->layer_param_.lstmv3_param();
        CHECK((lstmv3_param.has_num_cells()))
        << "lstmv3_param.has_num_cells()";
        CHECK((lstmv3_param.has_input_weight_filler()))
        << "lstmv3_param.has_input_weight_filler()";
        CHECK((lstmv3_param.has_input_gate_weight_filler()))
        << "lstmv3_param.has_input_gate_weight_filler()";
        CHECK((lstmv3_param.has_forget_gate_weight_filler()))
        << "lstmv3_param.has_forget_gate_weight_filler()";
        CHECK((lstmv3_param.has_output_gate_weight_filler()))
        << "lstmv3_param.has_output_gate_weight_filler()";

        channels_ = lstmv3_param.num_cells();
        CHECK_EQ(channels_, bottom[1]->shape(2)) <<
    ...
    template <typename Dtype>
    void LSTMV3Layer<Dtype>::Reshape(const vector<Blob<Dtype>*>& bottom,
                                   const vector<Blob<Dtype>*>& top) {
        ...
        input_gates_data_buffer_->Reshape(vector<int>{num_, channels_});
        forget_gates_data_buffer_->Reshape(vector<int>{num_, channels_});
        output_gates_data_buffer_->Reshape(vector<int>{num_, channels_});
        input_values_data_buffer_->Reshape(vector<int>{num_, channels_});
        gates_diff_buffer_->Reshape(vector<int>{num_, 4*channels_});
    ...
    template <typename Dtype>
    void LSTMV3Layer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
                                       const vector<Blob<Dtype>*>& top) {
        for (int i = 0; i < 2; ++i) {
        ...
        for (int n = 0; n < num_; ++n) {
            for (int i = 0; i < channels_; ++i) {
                ...
                if (this->layer_param_.lstmv3_param().tanh_hidden()) {
    ...
    template <typename Dtype>
    void LSTMV3Layer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
    STUB_GPU(LSTMV3Layer);
#endif

    INSTANTIATE_CLASS(LSTMV3Layer);
    REGISTER_LAYER_CLASS(LSTMV3);
