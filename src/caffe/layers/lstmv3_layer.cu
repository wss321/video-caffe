...
namespace caffe {
    ...
    template<typename Dtype>
    void LSTMV3Layer<Dtype>::Forward_gpu(const vector<Blob<Dtype> *> &bottom,
        ...
        CAFFE_CUDA_NUM_THREADS>>>(
                count,
                this->layer_param_.lstmv3_param().tanh_hidden(),
                prev_state_data,
                input_gates,
                forget_gates,
                output_gates,
    ...
    template<typename Dtype>
    void LSTMV3Layer<Dtype>::Backward_gpu(const vector<Blob<Dtype> *> &top,
...
// NOLINT_NEXT_LINE(whitespace/operators)
    ...
    INSTANTIATE_LAYER_GPU_FUNCS(LSTMV3Layer);
