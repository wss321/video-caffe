...
template<typename Dtype>
void Solver<Dtype>::TestIter(const int test_net_id) {
  /* Test One Iter*/
  CHECK(Caffe::root_solver());
  CHECK_NOTNULL(test_nets_[test_net_id].get())->
      ShareTrainedLayersWith(net_.get());
...
void Solver<Dtype>::Snapshot() {
  ...
  switch (param_.snapshot_format()) {
    case caffe::SolverParameter_SnapshotFormat_BINARYPROTO:model_filename = SnapshotToBinaryProto();
      break;
    case caffe::SolverParameter_SnapshotFormat_HDF5:model_filename = SnapshotToHDF5();
      break;
    default:LOG(FATAL) << "Unsupported snapshot format.";
  }
