...
namespace caffe {
    ...
    template<typename Dtype>
    __global__ void GRUForward(const int nthreads, const int H, const int t,
    ...
    template<typename Dtype>
    __global__ void GRUBackward(const int nthreads, const int H, const int t,
    ...
    void LSTMV2Layer<Dtype>::Forward_gpu(const vector<Blob<Dtype> *> &bottom,
        ...
        for (int t = 0; t < T_; ++t) {
            ...
            CUDA_POST_KERNEL_CHECK;
            GRUForward<Dtype><<<CAFFE_GET_BLOCKS(N_ * H_), CAFFE_CUDA_NUM_THREADS>>>(
    ...
    void LSTMV2Layer<Dtype>::Backward_gpu(const vector<Blob<Dtype> *> &top,
        ...
        for (int t = T_ - 1; t >= 0; --t) {
            ...
            GRUBackward<Dtype><<<CAFFE_GET_BLOCKS(N_ * H_), CAFFE_CUDA_NUM_THREADS>>>(
