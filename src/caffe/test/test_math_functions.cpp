...
TYPED_TEST_CASE
(CPUMathFunctionsTest, TestDtypes);
...
TYPED_TEST(CPUMathFunctionsTest, MatMul) {
  FillerParameter filler_param;
  ...
  a.Reshape({3, 5000, 4000});
  b.Reshape({4000, 5000});
  c.Reshape({3, 5000, 5000});
  filler.Fill(&a);
  filler.Fill(&b);
  long long start = systemtime_ms();
  caffe_cpu_gemm(CblasNoTrans,
                 CblasNoTrans,
                 3 * 5000,
                 5000,
                 4000,
...
TYPED_TEST_CASE
(GPUMathFunctionsTest, TestDtypes);
...
TYPED_TEST(GPUMathFunctionsTest, MatMul) {
  FillerParameter filler_param;
  GaussianFiller<float> filler(filler_param);
  Blob<float> a;
  Blob<float> b;
